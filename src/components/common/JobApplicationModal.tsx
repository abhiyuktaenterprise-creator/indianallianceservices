...
import { Send, CheckCircle2, Briefcase, Sparkles, AlertTriangle, Phone, MessageCircle } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (duplicateCheck.isDuplicate) {
      ...
      toast({
        ...
        description: `We already have an active application registered for +91 ${normalizePhoneNumber(formData.phone)}. Our HR team will call you shortly.`,
        variant: "destructive",
      });
      return;
    }
        ...
        {isSubmitted ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="app-phone" className="text-xs font-semibold text-foreground flex items-center justify-between">
            ...
            {duplicateInfo.isDuplicate && (
              <div className="rounded-2xl bg-amber-500/10 border-2 border-amber-500/40 p-4 text-left space-y-2.5 animate-in fade-in slide-in-from-top-2 duration-300 shadow-sm">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <h4 className="font-heading font-bold text-foreground text-sm">
                      You have already applied with this mobile number!
                    </h4>
                    <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
                      An application for mobile number <strong className="text-foreground font-semibold">+91 {normalizePhoneNumber(formData.phone)}</strong> was registered on <span className="text-foreground font-medium">{duplicateInfo.leadRecord?.submittedAt || "recently"}</span>{duplicateInfo.leadRecord?.targetRole ? ` for "${duplicateInfo.leadRecord.targetRole}"` : ""}.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-background/80 rounded-xl p-3 text-xs text-foreground/90 border border-amber-500/20 space-y-1">
                  <p className="font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                    ...
                    <span>Your application is currently under Talent Acquisition review</span>
                  </p>
                  <p className="text-[11px] text-muted-foreground pl-5">
                    Our Senior HR team will reach out for telephonic screening. No need to re-submit.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-1">
                  ...
                  >
                    ...
                    <span>Call HR Desk: {settings.helplinePhone}</span>
                  </a>
                  <a
                    href={`https://wa.me/${settings.whatsappPhone.replace(/[^0-9]/g, "")}?text=Hi%20Indian%20Alliance%20Services,%20I%20have%20already%20applied%20from%20%2B91${normalizePhoneNumber(formData.phone)}%20and%20need%20an%20update.`}
                    target="_blank"
            ...
            <Button
              ...
              className={`w-full gap-2 font-semibold h-11 transition-all ${
                duplicateInfo.isDuplicate
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <CheckCircle2 className="h-4 w-4" /> Already Applied — In Review Queue
                </>
              ) : (
                <>
